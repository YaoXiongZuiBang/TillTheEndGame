...
//        商店
        ...
        shopPanel.setBounds(800,50,backIcon.getIconWidth(),backIcon.getIconHeight());
        shopPanel.setLayout(null);
        panel.add(shopPanel);
        //商店物品
        for(int i=0;i<3;i++){
            for(int j=0;j<4;j++) {
                String filePath = "/img/equipment/equip_" + (i + 1) + ".png";
                Image goodsImg = Toolkit.getDefaultToolkit().getImage(getClass().getResource(filePath));
                JButton goodsButton = ButtonFactory.makeButton(goodsImg, 50+j*70, 100+i*80);
        ...
        JButton buyButton = ButtonFactory.makeButton(buyImg, 250, 580);
